export default function Footer() {
  return (
    <footer className="bg-[#0a5a2a] text-white pt-8 pb-20 lg:py-10 relative">
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
          {/* JKKN College of Pharmacy */}
          <div className="flex flex-col">
            <h3 className="text-base sm:text-lg font-bold mb-2 sm:mb-3 border-b-2 border-white pb-1.5 sm:pb-2">
              JKKN COLLEGE OF PHARMACY
            </h3>

            {/* Google Maps Embed - Responsive */}
            <div className="mb-2 w-full aspect-video-responsive rounded overflow-hidden">
              <iframe
                src="https://www.google.com/maps/embed?pb=!1m14!1m8!1m3!1d15641.93400155775!2d77.730733!3d11.444986!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x3ba9681dbc1ee209%3A0x3f44c8e40841c21b!2sJKKN%20College%20of%20Pharmacy!5e0!3m2!1sen!2sin!4v1770199740339!5m2!1sen!2sin"
                width="100%"
                height="100%"
                style={{ border: 0 }}
                allowFullScreen={false}
                loading="lazy"
                referrerPolicy="no-referrer-when-downgrade"
                className="rounded"
                title="JKKN College of Pharmacy Location"
              ></iframe>
            </div>

            <p className="text-[10px] xs:text-xs font-semibold mt-auto">
              Best Innovation-Focused<br />
              Multidisciplinary Campus
            </p>
          </div>

          {/* Our Institutions */}
          <div className="flex flex-col">
            <h3 className="text-base sm:text-lg font-bold mb-2 sm:mb-3 border-b-2 border-white pb-1.5 sm:pb-2">
              OUR INSTITUTIONS
            </h3>
            <ul className="space-y-2 sm:space-y-2.5 text-xs xs:text-sm">
              <li>
                <a
                  href="https://dental.jkkn.ac.in/"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="hover:text-gray-300 transition-colors inline-block"
                >
                  JKKN Dental College & Hospital
                </a>
              </li>
              <li>
                <a
                  href="https://ahs.jkkn.ac.in/"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="hover:text-gray-300 transition-colors inline-block"
                >
                  JKKN College of Allied Health Sciences
                </a>
              </li>
              <li>
                <a
                  href="https://pharmacy.jkkn.ac.in/"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="hover:text-gray-300 transition-colors inline-block"
                >
                  JKKN College of Pharmacy
                </a>
              </li>
              <li>
                <a
                  href="https://nursing.sresakthimayeil.jkkn.ac.in/"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="hover:text-gray-300 transition-colors inline-block"
                >
                  Sresakthimayeil Institute of Nursing and Research
                </a>
              </li>
              <li>
                <a
                  href="https://edu.jkkn.ac.in/"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="hover:text-gray-300 transition-colors inline-block"
                >
                  JKKN College of Education
                </a>
              </li>
              <li>
                <a
                  href="https://cas.jkkn.ac.in/"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="hover:text-gray-300 transition-colors inline-block"
                >
                  JKKN College of Arts and Science (Autonomous)
                </a>
              </li>
              <li>
                <a
                  href="https://engg.jkkn.ac.in/"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="hover:text-gray-300 transition-colors inline-block"
                >
                  JKKN College of Engineering and Technology
                </a>
              </li>
              <li>
                <a
                  href="https://school.jkkn.ac.in/"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="hover:text-gray-300 transition-colors inline-block"
                >
                  JKKN Matriculation Higher Secondary School
                </a>
              </li>
              <li>
                <a
                  href="https://nv.jkkn.ac.in/"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="hover:text-gray-300 transition-colors inline-block"
                >
                  Nattraja Vidhyalaya
                </a>
              </li>
            </ul>
          </div>

          {/* Contact Us */}
          <div className="flex flex-col sm:col-span-2 lg:col-span-1">
            <h3 className="text-base sm:text-lg font-bold mb-2 sm:mb-3 border-b-2 border-white pb-1.5 sm:pb-2">
              CONTACT US
            </h3>
            <div className="space-y-2 text-xs xs:text-sm mb-4">
              <p className="flex flex-col sm:flex-row sm:items-center gap-0.5 sm:gap-2">
                <strong className="font-semibold">Call:</strong>
                <a
                  href="tel:+919345855001"
                  className="hover:text-gray-300 transition-colors inline-block"
                >
                  +91 93458 55001
                </a>
              </p>
              <p className="flex flex-col sm:flex-row sm:items-center gap-0.5 sm:gap-2">
                <strong className="font-semibold">Email:</strong>
                <a
                  href="mailto:pharmacy@jkkn.ac.in"
                  className="hover:text-gray-300 transition-colors inline-block break-all"
                >
                  pharmacy@jkkn.ac.in
                </a>
              </p>
              <div>
                <strong className="font-semibold block mb-0.5">Address:</strong>
                <p className="leading-snug">
                  JKKN College of Pharmacy, Natarajapuram, NH-544 (Salem To Coimbatore National Highway), Komarapalayam (TK), Namakkal (DT), Tamil Nadu - 638183
                </p>
              </div>
            </div>

            {/* Follow Us */}
            <div className="mt-auto">
              <h3 className="text-base sm:text-lg font-bold mb-2 sm:mb-3 border-b-2 border-white pb-1.5 sm:pb-2">
                FOLLOW US
              </h3>
              <div className="flex flex-wrap gap-2">
                <a
                  href="https://www.facebook.com/jkknpharmacy"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-8 h-8 bg-white/10 rounded-full flex items-center justify-center hover:bg-white/20 transition-all duration-300"
                  aria-label="Follow JKKN College of Pharmacy on Facebook"
                >
                  <svg className="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                    <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                  </svg>
                </a>
                <a
                  href="https://www.instagram.com/jkknpharmacy"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-8 h-8 bg-white/10 rounded-full flex items-center justify-center hover:bg-white/20 transition-all duration-300"
                  aria-label="Follow JKKN College of Pharmacy on Instagram"
                >
                  <svg className="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                    <path d="M12 0C8.74 0 8.333.015 7.053.072 5.775.132 4.905.333 4.14.63c-.789.306-1.459.717-2.126 1.384S.935 3.35.63 4.14C.333 4.905.131 5.775.072 7.053.012 8.333 0 8.74 0 12s.015 3.667.072 4.947c.06 1.277.261 2.148.558 2.913.306.788.717 1.459 1.384 2.126.667.666 1.336 1.079 2.126 1.384.766.296 1.636.499 2.913.558C8.333 23.988 8.74 24 12 24s3.667-.015 4.947-.072c1.277-.06 2.148-.262 2.913-.558.788-.306 1.459-.718 2.126-1.384.666-.667 1.079-1.335 1.384-2.126.296-.765.499-1.636.558-2.913.06-1.28.072-1.687.072-4.947s-.015-3.667-.072-4.947c-.06-1.277-.262-2.149-.558-2.913-.306-.789-.718-1.459-1.384-2.126C21.319 1.347 20.651.935 19.86.63c-.765-.297-1.636-.499-2.913-.558C15.667.012 15.26 0 12 0zm0 2.16c3.203 0 3.585.016 4.85.071 1.17.055 1.805.249 2.227.415.562.217.96.477 1.382.896.419.42.679.819.896 1.381.164.422.36 1.057.413 2.227.057 1.266.07 1.646.07 4.85s-.015 3.585-.074 4.85c-.061 1.17-.256 1.805-.421 2.227-.224.562-.479.96-.899 1.382-.419.419-.824.679-1.38.9-.42.165-1.065.359-2.235.42-1.274.045-1.649.061-4.859.061-3.211 0-3.586-.015-4.859-.074-1.171-.061-1.816-.256-2.236-.421-.569-.224-.96-.479-1.379-.899-.421-.419-.69-.824-.9-1.38-.165-.42-.359-1.065-.42-2.235-.045-1.26-.061-1.649-.061-4.844 0-3.196.016-3.586.061-4.861.061-1.17.255-1.814.42-2.234.21-.57.479-.96.9-1.381.419-.419.81-.689 1.379-.898.42-.166 1.051-.361 2.221-.421 1.275-.045 1.65-.06 4.859-.06l.045.03zm0 3.678c-3.405 0-6.162 2.76-6.162 6.162 0 3.405 2.76 6.162 6.162 6.162 3.405 0 6.162-2.76 6.162-6.162 0-3.405-2.76-6.162-6.162-6.162zM12 16c-2.21 0-4-1.79-4-4s1.79-4 4-4 4 1.79 4 4-1.79 4-4 4zm7.846-10.405c0 .795-.646 1.44-1.44 1.44-.795 0-1.44-.646-1.44-1.44 0-.794.646-1.439 1.44-1.439.793-.001 1.44.645 1.44 1.439z"/>
                  </svg>
                </a>
                <a
                  href="https://www.linkedin.com/school/jkknpharmacy/"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-8 h-8 bg-white/10 rounded-full flex items-center justify-center hover:bg-white/20 transition-all duration-300"
                  aria-label="Connect with JKKN College of Pharmacy on LinkedIn"
                >
                  <svg className="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                    <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
                  </svg>
                </a>
                <a
                  href="https://www.youtube.com/playlist?list=PL6QsTq-__Hhs0tz4hErvZYmnD9uZ4A4Gm"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-8 h-8 bg-white/10 rounded-full flex items-center justify-center hover:bg-white/20 transition-all duration-300"
                  aria-label="Watch JKKN College of Pharmacy videos on YouTube"
                >
                  <svg className="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                    <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/>
                  </svg>
                </a>
              </div>
            </div>
          </div>
        </div>

        {/* Copyright */}
        <div className="mt-4 sm:mt-5 pt-3 sm:pt-4 border-t border-white/20 text-center">
          <p className="text-[10px] xs:text-xs">&copy; 2026 JKKN College of Pharmacy. All rights reserved.</p>
        </div>
      </div>
    </footer>
  )
}
